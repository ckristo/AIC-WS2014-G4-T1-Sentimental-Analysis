...
\alignauthor
Martin Kaufleitner\\
\affaddr{e1027229@student}\\
\affaddr{.tuwien.ac.at}\\
\affaddr{Vienna University of Technology}\\
\affaddr{Austria}\\
% 2nd. author
\alignauthor
Christoph Kindl\\
\affaddr{christoph.kindl@student}\\
\affaddr{.tuwien.ac.at}\\
\affaddr{Vienna University of Technology}\\
\affaddr{Austria}\\
\and
...
It's obvious that collecting many tweets in some particular fields by the Twitter API also take some problems into account: language's specifications like grammar or slang. Therefore we focused in our research on the English language.
...
Our own implementation seems to suffer from a lack of advanced processing features, with results being only marginally better than random. The best accuracy our implementation achieved was 55\% using a SVM machine learning approach and a training set of size 100k. We tried other machine learning approaches (kNN and Bayes) and an even bigger training set of size 200k, but couldn't improve the performance of the classifier (see the results of the table below).\\

\begin{tabular}{|l|c|c|c|c|}
	\hline
	classifier model / training set size & \textbf{10k} & \textbf{20k} & \textbf{100k} & \textbf{200k} \\
	\hline
	\textbf{SVM} & 52.6104\% & 52.008\% & 54.6185\% & 52.6104\% \\
	\hline
	\textbf{Bayes} & 46.988\% & 46.5863\% & 48.1928\% & 48.996\% \\
	\hline
	\textbf{kNN} & 41.5663\% & 43.1727\% & 47.7912\% & 48.5944\% \\
	\hline
\end{tabular}

(training data and test data taken from Sentiment140, see \url{http://help.sentiment140.com/for-students/}; test set size was 500 entries of manually-classified tweets) \\

We made several changes / additions to the preprocessing step in order to improve the results, but didn't evaluate each change we made because of costly classifier training (e.g. 200k with SVM took 1 day using a quad-core 3.40GHz machine). Due to the fact that the usage of other machine learning approaches and bigger training sets didn't improve our results that much, we should have included the preprocessing changes for performance evaluation. Furthermore, we used only one prepared training set (Sentiment140) -- maybe we should have tried others as well to improve our results.
