...
public class ApplicationConfig {
	 ...
	 */
	private static final String CONFIG_KEY_CLASSIFIER_TYPE = "classifier.type";
	...
	/**
	 * Constructor.
	 * @param inputStream the configuration properties file as input stream to load
	 * @throws IOException
	 */
	...
	/**
	 * Returns the configuration option that indicates whether to export the trained classifier to file (defaults to false).
	 * @return the configuration option that indicates whether to export the trained classifier to file (defaults to false).
	 */
	...
	/**
	 * Returns the configuration option that indicates whether to import a trained classifier from file (defaults to false).
	 * @return the configuration option that indicates whether to import a trained classifier from file (defaults to false).
	 */
	...
	public String getClassifierOutputDirectory() {
		return getProperty(CONFIG_KEY_CLASSIFIER_EXPORT_OUTPUT_DIRECTORY, ".");
	}
		...
		if (className != null) {
			try {
				...
				if (Classifier.class.isAssignableFrom(cls)) {
					return (Class<? extends Classifier>) cls;
				} else {
					logger.warn("Classifier type defined in application configuration is not valid (invalid class)");
				}
			} catch (ClassNotFoundException ex) {
				logger.warn("Classifier type defined in application configuration is not valid (unexisting class)", ex);
			}
		}
		return null;
	}
